...
async function loadClientDashboard(user) {
    try {
        ...
        if (response.ok) {
            ...
            if (visitsLeftEl) visitsLeftEl.textContent = clientData.visits_left;
            if (hasSubscriptionEl) hasSubscriptionEl.textContent = clientData.has_subscription ? 'Активен' : 'Неактивен';
        }
    } catch (error) {
